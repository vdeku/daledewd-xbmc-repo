...
class GoogleReader(object):
    ...
    def doSearch(self, searchString):
        resultIdsJson = self.httpGet( 'http://www.google.com/reader/api/0/search/items/ids', { 'output': 'json', 'q': searchString, 'num': '200', } )
        searchResultIds = json.loads(resultIdsJson, strict=False)['results']

        resultIds = []
        for resultId in searchResultIds:
            resultIds.append( ('i', resultId['id']) )
            
        resultItems = self.auth.post('http://www.google.com/reader/api/0/stream/items/contents', resultIds )
        return json.loads(resultItems, strict=False)['items']
        
    def getFeedItems(self, itemIds):
        lookupIds = []
        for itemId in itemIds:
            lookupIds.append( ('i', itemId) )
            
        resultItems = self.auth.post('http://www.google.com/reader/api/0/stream/items/contents', lookupIds )
        return json.loads(resultItems, strict=False)['items']
...
class ClientAuth(AuthenticationMethod):
    ...
    def __init__(self, username, password, auth_token=None, token=None):
    ...
    def postParameters(self, post=None):
        if post is not None:
            if type(post).__name__ == 'list':
                post.append(('T', self.token))
            elif type(post).__name__ == 'dict':
